...
function TermenSection({ initialFlashcards, assessments, onAssessmentsChange, onRepeatCountsChange, initialFlashcardRepeats, kennisBoosterTitel }) {
  ...
  const [showRepeatOnly, setShowRepeatOnly] = useState(false); // NIEUW: toggle voor 'te herhalen vragen'
  ...
  useEffect(() => {
    if (!isReviewing) {
      setActiveFlashcards(shuffleArray(getActiveCards()));
      setCurrentCardIndex(0);
      setShowAnswer(false);
      setReviewedThisRound([]);
      setShowRepeatDone(false);
    }
  }, [initialFlashcards, levelFilter, isReviewing, cardSideFilter, showRepeatOnly, assessments]);
...
Sluit af met een uitnodiging tot verdere verdieping: vraag wat de student nog lastig vindt aan het begrip, bied aan het op een andere manier uit te leggen (bijvoorbeeld met een metafoor of voorbeeld), of stel een verwante term voor om verder te verkennen.`;
  ...
  // Helper: centrale functie voor actieve kaarten (niveau, kaartzijde, reverse, te herhalen)
  function getActiveCards({
    includeRepeatOnly = false,
    forStats = false // voor PDF-certificaat: als true, gebruik alle kaarten, niet alleen actieve set
  } = {}) {
    const validInitialFlashcards = Array.isArray(initialFlashcards) ? initialFlashcards : [];
    // Altijd expanden naar reverse
    const expandedFlashcards = validInitialFlashcards.flatMap(card => {
      const cards = [];
      cards.push({ ...card, isReverse: false });
      if (card.reverse) {
        cards.push({ ...card, id: `${card.id}_reverse`, isReverse: true });
      }
      return cards;
    });
    ...
    // Filter op 'te herhalen vragen' indien toggle aan
    if (includeRepeatOnly || showRepeatOnly) {
      cardsToDisplay = cardsToDisplay.filter(card => assessments[card.id] === 'Niet' || assessments[card.id] === 'Redelijk');
    }
    // Voor PDF: als forStats true, negeer kaartzijde/niveau, geef alle kaarten terug
    if (forStats) {
  ...
  const startReview = (assessmentFilterType) => {
    const cardsToConsiderForReview = getActiveCards({ includeRepeatOnly: false });
  ...
  const cardsInCurrentLearningSet = !isReviewing ? activeFlashcards : getActiveCards();
  ...
  const cardsForCurrentFilterForCounts = getActiveCards();
    ...
    <div className="space-y-6">
      ...
      <div className="mb-4 p-4 bg-gray-100 rounded-lg">
        ...
        <div className="flex flex-wrap gap-6 items-end flex-col sm:flex-row">
          ...
          {/* Kaartzijde */}
          <div>
            <div className="text-xs font-semibold text-gray-500 mb-1 pl-1">Kaartzijde</div>
            <div className="flex gap-2 items-center">
              <button
                type="button"
                onClick={() => setInfoOpen((v) => !v)}
                className="rounded-full bg-blue-100 hover:bg-blue-200 text-blue-700 w-7 h-7 flex items-center justify-center border border-blue-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
              ...
              <button
                ...
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors border border-gray-300 ${cardSideFilter === 'both' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 hover:bg-blue-100'}`}
              >
                Beide kanten
              </button>
              <button
                onClick={() => setCardSideFilter('normal')}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors border border-gray-300 ${cardSideFilter === 'normal' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 hover:bg-blue-100'}`}
              >
                Alleen voorkant → achterkant
              </button>
              <button
                onClick={() => setCardSideFilter('reverse')}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors border border-gray-300 ${cardSideFilter === 'reverse' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 hover:bg-blue-100'}`}
              >
          ...
          {/* Status */}
          <div>
            <div className="text-xs font-semibold text-gray-500 mb-1 pl-1">Status</div>
            <div className="flex gap-2">
              <div className="relative group">
                <button
                  onClick={() => setShowRepeatOnly(v => !v)}
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors border border-gray-300 ${showRepeatOnly ? 'bg-orange-600 text-white border-orange-600' : (getActiveCards({ includeRepeatOnly: false }).some(card => assessments[card.id] === 'Niet' || assessments[card.id] === 'Redelijk') ? 'bg-white text-orange-800 hover:bg-orange-100 border-orange-300' : 'bg-white text-gray-400 border-gray-300')}`}
                  aria-label="Toon alleen te herhalen vragen"
                  type="button"
                >
                  Te herhalen vragen
                </button>
                <div className="absolute left-0 top-10 z-30 bg-white border border-orange-300 rounded-lg shadow-lg p-3 w-64 text-xs text-gray-800 hidden group-hover:block">
                  Toon alleen de vragen die je nog niet of nog niet helemaal goed kent (gescoord met 'Niet' of 'Redelijk').
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Als er geen kaarten zijn, toon alleen de melding */}
      {(!currentCard && activeFlashcards.length === 0) ? (
        <p className="text-center text-gray-700 mb-4">
          {initialFlashcards && initialFlashcards.length > 0 
            ? 'Geen flashcards gevonden voor deze combinatie van filters. Pas je filters aan om kaarten te zien.'
        ...
        levelFilter ? (
          <div>
            ...
            {currentCard && !showRepeatDone && (
              <div className={`border p-6 rounded-lg shadow-md min-h-[200px] flex flex-col justify-between items-center mb-4 relative ${currentCard.isReverse ? 'bg-blue-50 border-blue-400' : 'bg-yellow-50 border-yellow-400'}`}>
